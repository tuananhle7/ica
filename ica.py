...
def generate_timeseries_signal(key, num_timesteps=500):
    ...
    return (
        true_source,
        mixing_matrix,
        jax.vmap(get_signal, (None, 0), 0)(mixing_matrix, true_source),
    )
...
def main():
    ...
    # Generate timeseries signal
    key = jax.random.PRNGKey(0)
    key, subkey = jax.random.split(key)
    true_source, mixing_matrix, signal = generate_timeseries_signal(subkey)
    ...
    # )
    total_log_likelihoods, raw_mixing_matrices, preprocessing_params = ica(
        key, signal, get_supergaussian_log_prob, num_iterations=num_iterations, lr=lr
    )
